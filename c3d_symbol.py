...
def get_symbol(num_classes):
    ...
    ## 1st group 
    conv1 = mx.symbol.Convolution(data=input_data, kernel=(3,3,3), stride=(1,1,1), pad=(1,1,1), num_filter=64, name='conv1', cudnn_tune='fastest')
    relu1 = mx.symbol.Activation(data=conv1, act_type='relu')
    pool1 = mx.symbol.Pooling(data=relu1, pool_type='max', kernel=(1,2,2), stride=(1,2,2))
    
    ## 2nd group
    conv2 = mx.symbol.Convolution(data=pool1, kernel=(3,3,3), stride=(1,1,1), pad=(1,1,1), num_filter=128, name='conv2', cudnn_tune='fastest')
    relu2 = mx.symbol.Activation(data=conv2, act_type='relu')
    pool2 = mx.symbol.Pooling(data=relu2, pool_type='max', kernel=(2,2,2), stride=(2,2,2))

    ## 3rd group
    conv3a = mx.symbol.Convolution(data=pool2, kernel=(3,3,3), stride=(1,1,1), pad=(1,1,1), num_filter=256, cudnn_tune='fastest')
    ...
    ## 4th group
    conv4a = mx.symbol.Convolution(data=pool3b, kernel=(3,3,3), stride=(1,1,1), pad=(1,1,1), num_filter=256, cudnn_tune='fastest')
    ...
    ## 5th group
    conv5a = mx.symbol.Convolution(data=pool4b, kernel=(3,3,3), stride=(1,1,1), pad=(1,1,1), num_filter=256, cudnn_tune='fastest')
